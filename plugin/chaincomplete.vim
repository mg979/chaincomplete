...
command! -bar           ChainCompleteAutoEnable  call v:lua.chaincomplete.auto.enable(1)

" disable completion for current buffer
command! -bar           ChainCompleteAutoDisable call v:lua.chaincomplete.auto.disable(1)

" toggle completion for current buffer
command! -bar           ChainCompleteAutoToggle  call v:lua.chaincomplete.auto.toggle()

" enable completion triggered by dot or ->
command! -bang -nargs=? ChainCompleteAutoTrigger call v:lua.chaincomplete.dot.init(<bang>0, <q-args>)

" show/set/reset chain
command! -bang -nargs=? ChainComplete            call v:lua.chaincomplete.set(<bang>0, <q-args>)
